...
function Category__({ name, pathAsArray, pathAsString, categoryRename, categoryObject }) {
    ...
    function sendToRename() {
        categoryRename(pathAsString + '.' + name, editedName);
        setOnEdit(!onEdit)
    }
        ...
        <Category_>
            ...
            <input type='text' readOnly={!onEdit} ref={inputRef} value={editedName} style={{ width: (editedName.length * 0.95 + 1) + 'ch' }} onChange={(e) => setEditedName(e.target.value.replace(/[`~!@#$%^&*()_|+\-=?;:'".<>\{\}\[\]\\\/]/gi, ''))}
                onClick={
                    () => {
                        if (!onEdit)
                        router.push('/categoryManager/'+pathAsArray.concat(name).join('/'));
                    }
                }
                onKeyDown={onEnter}
            />
...
export default connect(null, mapDispatchToProps)(Category__);
